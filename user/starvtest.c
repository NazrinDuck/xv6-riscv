...
void test_dona() {
  ...
  pids[PROC_NUM - 1] = fork();
  if (pids[PROC_NUM - 1] == 0) {
    close(pipe_fd[1]);
    // Child PROC_NUM - 1
    setpriority(getpid(), pids_prio_s[PROC_NUM - 1]);
    sched_setaffinity(getpid(), pids_cpu[PROC_NUM - 1]);

    for (int j = 0; j < BIG_NUM; ++j) {
      printf("%c\r", 'A' + PROC_NUM - 1);
    }
